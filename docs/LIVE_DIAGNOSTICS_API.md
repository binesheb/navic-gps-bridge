...
- `data_fresh` is `true` only after at least one GNSS packet has been received and the latest packet is less than 3000 ms old.
- `data_age_ms` is the unsigned `millis()` elapsed time since the latest received packet.
- Before the first packet, `data_fresh` is `false` and `data_age_ms` is `0`.
...
2. `data_fresh` and `data_age_ms` for the latest packet age.
3. `fix` for navigation validity.
4. `geofence_inside`, `geofence_events`, and `geofence_last_event_age_ms` for boundary state/history.

Do not interpret `data_age_ms == 0` as a fresh packet by itself; check `data_fresh` as well because `0` also represents the pre-first-packet state.

...
static constexpr double GEO_DEG_TO_RAD = 0.017453292519943295;

double GeofenceEngine::distance(double lat1,double lon1,double lat2,double lon2) const {
  double dLat=(lat2-lat1)*GEO_DEG_TO_RAD,dLon=(lon2-lon1)*GEO_DEG_TO_RAD;
  double a=sin(dLat/2)*sin(dLat/2)+cos(lat1*GEO_DEG_TO_RAD)*cos(lat2*GEO_DEG_TO_RAD)*sin(dLon/2)*sin(dLon/2);
  // Floating-point round-off can push a infinitesimally outside [0, 1].
  // Clamp before sqrt() so a valid boundary calculation never becomes NaN.
  if (a < 0.0) a = 0.0;
  if (a > 1.0) a = 1.0;

...
For the first bench connection of a physical receiver, use the [hardware bring-up runbook](docs/HARDWARE_BRINGUP.md). It establishes a receiver-only baseline before bridge diagnosis, records wiring/configuration identity, and defines a conservative troubleshooting order so electrical/UART problems are not misclassified as software or NavIC failures.

Before starting a direct receiver capture, use `tools/hardware_port_preflight.py` to verify that the field machine sees the expected serial port and that it can be opened at the requested baud. The preflight never transmits bytes and can save a JSON verdict:

```bash
python tools/hardware_port_preflight.py COM5 \
  --baud 9600 \
  --json-output evidence/bridge-01-receiver-a/port-preflight.json
```

A failed preflight returns a non-zero exit code and reports whether the port was missing, could not be opened, or the baud was invalid. This is intentionally a connection-readiness check, not evidence that the receiver is producing valid NMEA.

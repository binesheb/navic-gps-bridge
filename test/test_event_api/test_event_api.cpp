...
void test_cleared_history_has_no_latest_event() {
  ...
  data.hdop = 1.2f;
  engine.update(data, true, 1234);
  TEST_ASSERT_EQUAL_UINT(1, engine.history().size());
...
void setup() {
  ...
  RUN_TEST(test_cleared_history_has_no_latest_event);
  UNITY_END();
}
